...
\begin{block}
\noindent
\textsl{Given $d$ define \emph{\bf{$d_{1}$ fixed}} to be $\frac{d}{\lambda}$. Define $\mathcal{A}$ to be a \emph{\bf{set of generators}}, $\mathcal{A} = \{ (a_{i}) \vert a_{i} < a_{i+1},  \forall i \in [0, k-1] \} $, and $\vert \mathcal{A} \vert = k$. Also define a set of \emph{\bf{non-negative coefficients}} $c_{1}, c_{2}, .. , c_{k}$ such that $a_{i+1} = \alpha_{i}a_{i} \lambda$.\n
