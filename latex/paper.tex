...
\begin{abstract}
For positive integers $d$ and $k$ let $m(d,k)$ be defined to be the maximum modulus m such that the \emph{Cayley digraph} Cay($\mathbb{Z}_{m}, \mathcal{A}$) generated by the set of positive integers $\mathcal{A}$ of cardinality $k$ is such that the diameter of Cay($\mathbb{Z}_{m}, \mathcal{A}$) is less than or equal to a given diameter $d$. 
...
The focus of this paper is the finite group $\mathbb{Z}_m$; we will denote the Cayley graph Cay($mathbb{Z}_{m}, \mathcal{A}$) as Cay($m, \mathcal{A}$).
...
\[
m(d,\mathcal{A}) =\ max\{m | \diam(Cay(m,\mathcal{A})) \leq d\},
\]
...
\[
m(d,k) = \max\{m(d,\mathcal{A}) | \text{ there exists a set A with } \vert \mathcal{A} \vert = k \},
\]
