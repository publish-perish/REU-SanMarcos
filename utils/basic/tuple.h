...
bool operator<(const Tuple<TP, N>& lhs, const Tuple<TP, N>& rhs) {
    Tuple<TP, N> zero;
    if((rhs.size() == lhs.size()) && (lhs == rhs == zero)) return false;
...
bool operator<=(const Tuple<TP, N>& lhs, const Tuple<TP, N>& rhs) {
    Tuple<TP, N> zero;
    if((rhs.size() == lhs.size()) && (lhs == rhs == zero)) return true;
...
template<typename TP, int N>
bool Tuple<TP, N>::operator>=(const Tuple<TP, N>& rhs) {
    if(rhs.size() == this->size()) return gteq(*this, rhs);
}
